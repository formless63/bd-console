...
// --- shutdown ---------------------------------------------------------------
// The /api/events SSE streams (lib/events.mjs) are responses that never end by
// design, which makes them open sockets that no browser has any reason to
// close. Before this, exit was immediate because nothing held the process open;
// now a stop MUST hang the streams up itself, or `bd-console stop` (and
// systemd's SIGTERM) would wait on tabs.
//
// Deliberately blunt: close the streams, stop accepting, exit as soon as the
// server reports itself closed, and exit anyway after a short grace window so a
// wedged in-flight request can never turn a stop into a hang. Node >= 22's
// server.close() also closes idle keep-alive connections, which is what makes
// the fast path fast.
