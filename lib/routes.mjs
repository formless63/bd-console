...
import { loadRegistry, registerProjectPath, normalizeUserPath } from './registry.mjs';
import {
...
import { getDocs, resolveDocPath, validateDocRoots } from './docs.mjs';
import { listSessions, capturePane, sendPrompt, createSession, waitAndSendPrompt, SESSION_NAME_RE } from './tmux.mjs';
import { writeUploadedFiles } from './uploads.mjs';
import {
...
// POST /api/files/upload and POST /api/tmux/create (which can carry the same
// `files` array to attach before launch) both encode file content as base64
// JSON — roughly 4/3 the decoded size — so the body ceiling has to clear
// lib/uploads.mjs's UPLOAD_TOTAL_MAX_BYTES (12MB decoded) plus that overhead
// plus JSON/path framing.
const UPLOAD_BODY_MAX_BYTES = 17 * 1024 * 1024;
...
// Tokenless LAN mode intentionally has no login ceremony, but a browser on a
// different site must not be able to turn that convenience into a CSRF write.
// Requiring JSON blocks ordinary cross-site forms, while comparing Origin to
// Host rejects cross-origin fetches that do send JSON. Native LAN clients stay
// frictionless: they only need the same JSON content type they already use.
export function csrfSafe(req) {
  ...
  try {
    ...
    // Matching Origin and Host is necessary but not sufficient against DNS
    // rebinding: an attacker's hostname can be made to resolve to this LAN
    // address while both headers still agree. Accept local interface names/IPs
    // (including the usual .local names), or a host explicitly designated for
    // a reverse proxy. Proxies normally add X-Forwarded-For; direct browsers
    // cannot set that non-simple header without a preflight.
    const hostName = parsed.hostname.toLowerCase();
    const localNames = new Set(['localhost', hostname().toLowerCase()]);
    for (const addresses of Object.values(networkInterfaces())) {
      for (const address of addresses || []) localNames.add(String(address.address).toLowerCase());
    }
    const configured = String(process.env.BD_CONSOLE_TRUSTED_HOSTS || '')
      .split(',').map((v) => v.trim().toLowerCase()).filter(Boolean);
    // Forwarded headers are caller-controlled unless the TCP peer is the
    // local gateway. Never let a rebinding page opt itself into proxy trust by
    // adding X-Forwarded-For. The maintainer's Pangolin/newt path terminates
    // locally, so this stays configuration-free for the normal deployment.
    const remote = String(req.socket?.remoteAddress || '').toLowerCase();
    const localProxy = remote === '::1' || remote === '127.0.0.1' || remote === '::ffff:127.0.0.1';
    return localNames.has(hostName) || hostName.endsWith('.local')
      || configured.includes(hostName) || localProxy;
  } catch {
    return false;
  }
}
...
// File/session creation receives an absolute directory from the browser. Keep
// that control plane inside a workspace the hub owner explicitly registered;
// this preserves the project-folder workflow while preventing a LAN browser
// from turning the endpoint into arbitrary filesystem access. Existing and
// missing paths are checked by realpath, so a symlink cannot jump outside the
// registered root.
function registeredWorkspaceContains(inputPath) {
  const candidate = resolve(inputPath);
  let existing = candidate;
  let candidateReal;
  while (true) {
    try { candidateReal = realpathSync(existing); break; } catch {
      const parent = dirname(existing);
      if (parent === existing) return false;
      existing = parent;
    }
  }
  for (const entry of Object.values(loadRegistry().projects || {})) {
    if (!entry?.path) continue;
    try {
      const rootReal = realpathSync(entry.path);
      if (pathInside(rootReal, candidateReal)) return true;
    } catch { /* stale registrations do not grant a filesystem root */ }
  }
  return false;
}
...
function getContext(reqPath) {
  ...
  const configDocRoots = validateDocRoots(workspace, c.docRoots);
...
async function cachedBdVersion(ctx) {
  ...
  // `bd version` is a property of the executable, not of a project. Do not
  // run it with a registered workspace cwd: a deleted project would produce
  // ENOENT and poison this process-wide cache for every healthy project.
  const r = await run('bd', ['version'], { maxBuffer: 1024 * 1024, timeout: 10000 });
  const result = { ok: r.ok, stdout: r.stdout, stderr: r.stderr || (r.code === 'ENOENT' ? 'bd binary not found on the daemon\'s PATH' : '') };
...
export function createRequestHandler({ host, port, token, argsHost = null, argsPort = null }) {
  function authed(req, url) {
    ...
    // Never accept credentials in the URL: browser history, reverse-proxy
    // access logs and Referer headers routinely retain query strings.
    const supplied = req.headers['x-bd-token'];
    if (typeof supplied !== 'string') return false;
    const actual = createHash('sha256').update(supplied).digest();
    const expected = createHash('sha256').update(String(token)).digest();
  ...
  async function computeHealth(ctx, exportInfo) {
    ...
    if (!existsSync(ctx.workspace)) {
      errors.push(`project directory no longer exists at ${ctx.workspace} — remove it from the hub or restore the path`);
    }
    let bdVersion = null;
  ...
  return async function handler(req, res) {
    try {
      ...
      if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(req.method) && !csrfSafe(req)) {
        return sendJson(res, 403, { error: 'unsafe requests must use application/json and a same-origin Host/Origin' });
      }
      ...
      if (originalPath === '/api/settings' && req.method === 'GET') {
        ...
        return sendJson(res, 200, {
          ...
          // set/opaque-mask/source, never the value or even a prefix; the browser composes
      ...
      if (originalPath === '/api/settings' && req.method === 'POST') {
        ...
        // Validate and stage the complete patch before touching disk. The old
        // route saved token/defaultEpics/Termix independently, so a malformed
        // later field left an earlier field persisted despite a 400 response.
        // One saveGlobalConfig call gives this request all-or-nothing config
        // publication (saveGlobalConfig itself uses temp+rename).
        const globalPatch = {};
        const globalUnset = [];
        ...
        if ('defaultEpics' in body) {
          ...
          globalPatch.defaultEpics = merged;
        }
        ...
        if ('termix' in body) {
          ...
          const currentTermix = loadGlobalConfig().termix;
          const baseTermix = (currentTermix && typeof currentTermix === 'object' && !Array.isArray(currentTermix))
            ? currentTermix : {};
          const nextTermix = { ...baseTermix, ...termixPatch };
          for (const key of termixUnset) delete nextTermix[key];
          if (Object.keys(nextTermix).length) globalPatch.termix = nextTermix;
          else globalUnset.push('termix');
        }

        try {
          saveGlobalConfig(globalPatch, globalUnset);
        } catch (e) {
      ...
      if (originalPath === '/api/prompts' && req.method === 'GET') {
        // Prompt text can contain credentials or other private instructions;
        // keep it behind the optional token when one is configured, while the
        // tokenless LAN mode remains frictionless.
        if (!authed(req, url)) return sendJson(res, 401, { error: 'token required' });
      ...
      // ---- new code session launcher (bd-console-cox) ------------------------
      // "New code session": pick/create a folder, name a tmux session, launch
      // `claude --remote-control <name>` in it, optionally drop files into
      // the folder first and send a starter prompt once the pane is up.
      // Token-gated like every other write; the launch command itself is
      // fixed (never taken from the request body) — see createSession()'s
      // header in lib/tmux.mjs for why that matters here specifically.
      if (originalPath === '/api/tmux/create' && req.method === 'POST') {
        if (!authed(req, url)) return sendJson(res, 401, { error: 'token required' });
        const body = await readBody(req, UPLOAD_BODY_MAX_BYTES).catch(() => null);
        if (!body) return sendJson(res, 400, { error: 'bad body' });

        const name = String(body.name || '').trim();
        if (!SESSION_NAME_RE.test(name)) {
          return sendJson(res, 400, { error: 'session name must contain only letters, numbers, and . _ : @ -' });
        }

        const norm = normalizeUserPath(body.dir);
        if (!norm.ok) return sendJson(res, norm.status, { error: norm.error });
        const dir = norm.path;

        if (body.createDir === true) {
          // Check the eventual parent before creating a new directory; a
          // rejected path must not leave an out-of-scope folder behind.
          if (!registeredWorkspaceContains(dir)) {
            return sendJson(res, 403, { error: 'directory must be inside a registered project workspace' });
          }
          try { await mkdir(dir, { recursive: true }); }
          catch (e) { return sendJson(res, 400, { error: `could not create ${dir}: ${e.message}` }); }
        } else {
          let dirStat;
          try { dirStat = await stat(dir); } catch { dirStat = null; }
          if (!dirStat || !dirStat.isDirectory()) {
            return sendJson(res, 400, { error: `directory does not exist: ${dir} (enable "create it" to make a new project folder)` });
          }
        }
        if (!registeredWorkspaceContains(dir)) {
          return sendJson(res, 403, { error: 'directory must be inside a registered project workspace' });
        ...
        if (Array.isArray(body.files) && body.files.length > 0) {
          const uploadResult = await writeUploadedFiles(dir, body.files, { overwrite: body.overwrite === true });
          if (!uploadResult.ok) return sendJson(res, uploadResult.status, { error: uploadResult.error });
          written = uploadResult.written;
          skipped = uploadResult.skipped;
        }

        const created = await createSession(name, dir);
        if (!created.ok) return sendJson(res, created.status || 400, { error: created.error });

        const prompt = typeof body.prompt === 'string' ? body.prompt.trim() : '';
        let promptSent = null;
        let promptError = null;
        if (prompt) {
          const sent = await waitAndSendPrompt(name, prompt);
          promptSent = sent.ok;
          if (!sent.ok) promptError = sent.error;
        }

        return sendJson(res, 200, { ok: true, name, dir, written, skipped, promptSent, promptError });
      }

      // Shared with the `files` step of POST /api/tmux/create above, but
      // reachable on its own — "drop a file into an already-registered
      // project" (bd-console-cox.2/cox.4) doesn't need a session launch.
      if (originalPath === '/api/files/upload' && req.method === 'POST') {
        if (!authed(req, url)) return sendJson(res, 401, { error: 'token required' });
        const body = await readBody(req, UPLOAD_BODY_MAX_BYTES).catch(() => null);
        if (!body) return sendJson(res, 400, { error: 'bad body' });
        ...
        const norm = normalizeUserPath(body.dir);
        if (!norm.ok) return sendJson(res, norm.status, { error: norm.error });
        // Let the upload helper retain its established 400 for a missing
        // directory; scope-check existing paths before any file is written.
        if (existsSync(norm.path) && !registeredWorkspaceContains(norm.path)) {
          return sendJson(res, 403, { error: 'directory must be inside a registered project workspace' });
        }

        const result = await writeUploadedFiles(norm.path, body.files, { overwrite: body.overwrite === true });
        if (!result.ok) return sendJson(res, result.status, { error: result.error });
        return sendJson(res, 200, { ok: true, dir: norm.path, written: result.written, skipped: result.skipped });
      }
      ...
      if (originalPath.startsWith('/api/')) {
        ...
        if (path === '/api/comments') {
          ...
          if (!r.ok) return sendJson(res, 502, { error: r.stderr || r.stdout || 'bd comments failed' });
          let comments;
          try { comments = JSON.parse(r.stdout || '[]'); } catch {
            return sendJson(res, 502, { error: 'bd comments returned invalid JSON' });
          }
          if (!Array.isArray(comments)) return sendJson(res, 502, { error: 'bd comments returned an invalid response' });
        ...
        if (req.method === 'POST') {
          ...
          if (path === '/api/doc') {
            ...
            try {
              ...
              if (body.create === true) {
                // rename() would replace a file that appeared after the
                // existsSync check. A hard-link publish is atomic and fails
                // with EEXIST if another writer wins the create race.
                await link(tmp, full);
                await unlink(tmp);
              } else {
              ...
              if (body.create === true && e?.code === 'EEXIST') {
                return sendJson(res, 409, { error: 'a document already exists at that path' });
              }
          ...
          if (path === '/api/comment') {
            ...
            if (!cr.ok) return sendJson(res, 502, { error: cr.stderr || cr.stdout || 'comment was saved, but comments could not be refreshed' });
            let comments;
            try { comments = JSON.parse(cr.stdout || '[]'); } catch {
              return sendJson(res, 502, { error: 'comment was saved, but comments returned invalid JSON' });
            }
            if (!Array.isArray(comments)) return sendJson(res, 502, { error: 'comment was saved, but comments returned an invalid response' });
