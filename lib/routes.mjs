...
import {
  ...
  TERMIX_KEYS, normalizeTermixBaseUrl, validateTermixToken, resolveTermix, saveTermixConfig,
} from './config.mjs';
import { CONFIG_PATH } from './paths.mjs';
import {
  bd, getIssues, getIssueById, getExportInfo, ensureIssuesExportFresh,
  runIssueEdit, ID_RE, LABEL_RE, ASSIGNEE_RE,
...
import { getClaudeUsage, getCodexUsage, getKimiUsage } from './usage.mjs';
    ...
    try {
      ...
      if (originalPath === '/api/settings' && req.method === 'GET') {
        ...
        return sendJson(res, 200, {
          ...
          // Termix linkage — stored address + credential only; the server
          // never contacts it (see lib/config.mjs). The token follows the
          // write token's treatment exactly: set/masked-prefix/source, never
          // the value.
          termix: {
      ...
      if (originalPath === '/api/settings' && req.method === 'POST') {
        ...
        // Termix: address + credential storage only. Omitted sub-keys are
        // left alone; an explicit null/'' clears one. Nothing is restarted
        // and nothing is contacted — no code path reads these yet.
        if ('termix' in body) {
          const t = body.termix;
          ...
          try {
            for (const key of TERMIX_KEYS) {
              ...
              termixPatch[key] = key === 'baseUrl' ? normalizeTermixBaseUrl(t[key]) : validateTermixToken(t[key]);
            }
          } catch (e) {
      ...
      if (originalPath === '/api/usage' && req.method === 'GET') {
        ...
        // kimi is a stack-info provider, not a quota one (Kimi Code publishes
        // no rate-limit data) — it rides along here because it answers the same
        // "what is this machine's AI stack doing right now" question the hub's
        // Live-quota card asks, and it degrades to `not-installed` on machines
        // without it, exactly like codex does.
        const [claude, codex, kimi] = await Promise.all([getClaudeUsage({ fresh }), getCodexUsage(), getKimiUsage()]);
        return sendJson(res, 200, { providers: { claude, codex, kimi } });
      }
