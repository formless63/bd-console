...
//
// BOUNDED, because this is a long-lived daemon: the entries hold every parsed
// usage record of every transcript ever scanned, and transcripts only
// accumulate (Claude Code writes one per session, Codex one per session-day),
// so an unbounded map grew for the lifetime of the process and never gave a
// byte back. Insertion-ordered eviction of the least recently USED entry (a
// re-set moves an entry to the end) keeps the working set — the recent days a
// bounded `days` window actually reads — and drops the cold tail.
const MAX_FILE_CACHE_ENTRIES = 512;
const claudeFileCache = new Map();
const codexFileCache = new Map();

function cacheGet(cache, path) {
  const hit = cache.get(path);
  if (hit === undefined) return undefined;
  cache.delete(path);
  cache.set(path, hit); // touch: most-recently-used goes last
  return hit;
}

function cacheSet(cache, path, value) {
  cache.delete(path);
  cache.set(path, value);
  for (const oldest of cache.keys()) {
    if (cache.size <= MAX_FILE_CACHE_ENTRIES) break;
    cache.delete(oldest);
  }
}
...
async function getClaudeFileRecords(path, st, projectId) {
  const cached = cacheGet(claudeFileCache, path);
  if (cached && cached.mtimeMs === st.mtimeMs && cached.size === st.size) return cached.records;
  let text;
  try { text = await readTextCapped(path, st.size); } catch { claudeFileCache.delete(path); return []; }
  const records = parseClaudeFile(text, projectId);
  cacheSet(claudeFileCache, path, { mtimeMs: st.mtimeMs, size: st.size, records });
  return records;
}
...
async function computeClaudeHistory(from, now) {
  ...
  for (const pe of projectEntries) {
    ...
    for (const file of files) {
      ...
      // Gone (deleted, or a permission change): forget whatever we parsed for
      // it — a stat failure is the only signal this cache ever gets that a
      // path it holds is no longer real.
      if (!st) { claudeFileCache.delete(file); continue; }
...
async function getCodexFileRecords(path, st) {
  const cached = cacheGet(codexFileCache, path);
  if (cached && cached.mtimeMs === st.mtimeMs && cached.size === st.size) return cached.records;
  let text;
  try { text = await readTextCapped(path, st.size); } catch { codexFileCache.delete(path); return []; }
  const records = parseCodexFile(text);
  cacheSet(codexFileCache, path, { mtimeMs: st.mtimeMs, size: st.size, records });
  return records;
}
  ...
  for (const year of years) {
    ...
    for (const month of months) {
      ...
      for (const day of days) {
        ...
        for (const file of files) {
          const st = await safeStat(file);
          if (!st) { codexFileCache.delete(file); continue; } // gone — see the claude scan
