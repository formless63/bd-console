...
async function supersedeExisting({ host, port, willReplaceUnit = false }) {
  const notes = [];
  ...
  if (wasSystemdActive) {
    ...
    if (!stopUnit) {
      ...
      stopUnit = !!(unitPid && probe.reachable && probe.ok
        && looksLikeBdConsole(probe.data) && probe.data.pid === unitPid);
    }
    if (stopUnit) {
...
export async function daemonStart({ host, port, persist, forwardArgs = [], serveEntry }) {
  const { notes, wasSystemdActive } = await supersedeExisting({ host, port, willReplaceUnit: !!persist });
