...
import {
  DEFAULT_HOST, DEFAULT_PORT, isFirstRun, resolveSettings, saveGlobalConfig,
  normalizeTermixBaseUrl, validateTermixToken, validateTermixHostId,
...
// Nested under `termix` in config.json, so these take the dotted form on the
// CLI (`bd-console settings set termix.baseUrl https://…`) and route through
// config.mjs's saveTermixConfig rather than the flat saveGlobalConfig. They
// are listed and settable like everything else so "where does this live?" has
// one answer; nothing here contacts the Termix server.
const TERMIX_CLI_KEYS = ['termix.baseUrl', 'termix.token', 'termix.hostId'];
const ALL_KEYS = [...KEYS, ...TERMIX_CLI_KEYS];

// A credential is a credential: the same 4-char prefix treatment the write
// token gets, for the same reason.
const SECRET_KEYS = new Set(['token', 'termix.token']);
...
function validateAndCoerce(key, rawValue) {
  ...
  switch (key) {
    ...
    case 'termix.baseUrl':
      return normalizeTermixBaseUrl(raw);
    case 'termix.token':
      return validateTermixToken(raw);
    case 'termix.hostId':
      return validateTermixHostId(raw);
    default:
      throw new Error(`unknown setting: ${key} (expected one of ${ALL_KEYS.join(', ')})`);
  }
}
...
export function printSettingsList() {
  ...
  const values = {
    ...Object.fromEntries(KEYS.map((k) => [k, { value: s[k], source: s.sources[k] }])),
    'termix.baseUrl': { value: termix.baseUrl, source: termix.sources.baseUrl },
    'termix.token': { value: termix.token, source: termix.sources.token },
    'termix.hostId': { value: termix.hostId, source: termix.sources.hostId },
  };
  console.log('key              value                   source');
  for (const key of ALL_KEYS) {
    const { value, source } = values[key];
    console.log(`${key.padEnd(17)}${formatValue(key, value).padEnd(24)}${source}`);
  }
}
...
export async function settingsSet(key, value) {
  ...
  if (!ALL_KEYS.includes(key)) throw new Error(`unknown setting: ${key} (expected one of ${ALL_KEYS.join(', ')})`);
  if (value === undefined) throw new Error(`usage: bd-console settings set ${key} <value>`);
  const coerced = validateAndCoerce(key, value);
  const isTermix = TERMIX_CLI_KEYS.includes(key);
  if (isTermix) saveTermixConfig({ [key.slice('termix.'.length)]: coerced });
  else saveGlobalConfig({ [key]: coerced });
  console.log(`Saved ${key} = ${SECRET_KEYS.has(key) ? 'set' : coerced} to ${CONFIG_PATH}`);
  // termix.* is re-read from disk on every request, so a running server picks
  // it up without a restart — don't print advice that isn't true.
  if (!isTermix) await noteRestartIfRunning();
}
...
export async function settingsUnset(key) {
  ...
  if (!ALL_KEYS.includes(key)) throw new Error(`unknown setting: ${key} (expected one of ${ALL_KEYS.join(', ')})`);
  const isTermix = TERMIX_CLI_KEYS.includes(key);
  if (isTermix) saveTermixConfig({}, [key.slice('termix.'.length)]);
  else saveGlobalConfig({}, [key]);
