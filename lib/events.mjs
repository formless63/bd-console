...
async function sweep() {
  // setInterval does not await an async callback. Keep a slow filesystem
  // sweep from overlapping the next one and racing its stamp updates.
...
async function startSweeper() {
  if (sweeper) return;
  // Multiple clients can subscribe in the same turn. Share the async startup
  // promise so they cannot each seed and install their own interval.
  if (sweeperStarting) return sweeperStarting;
  sweeperStarting = (async () => {
    ...
    await sweep().catch(() => {});
    if (!clients.size || sweeper) return; // clients left while seeding
    sweeper = setInterval(() => { sweep().catch(() => {}); }, SWEEP_MS);
    if (typeof sweeper.unref === 'function') sweeper.unref();
  })().finally(() => { sweeperStarting = null; });
  return sweeperStarting;
}
