...
function runTmux(args, opts = {}) {
  return run('tmux', args, { maxBuffer: 8 * 1024 * 1024, timeout: 10000, ...opts });
}
...
export async function hasSession(name) {
  ...
  // tmux targets are prefix-matched by default (`has-session -t foo` also
  // succeeds for `foobar`). The `=` form is tmux's exact-target syntax; every
  // operation that accepts a session name must use it or a short name can
  // silently operate on another session.
...
// createSession(name, cwd, { launch }): the "New code session" launcher
// (bd-console-cox.1). Spawns a detached tmux session at `cwd` running
// `launch` (default `claude --remote-control <name>`) and dropping into the
// user's shell afterward, so exiting or Ctrl-C'ing the agent leaves a normal
// shell behind rather than killing the pane — the same shape as the
// hand-run `tmux new -d -s NAME -c DIR 'claude --remote-control NAME; exec
// $SHELL'` this feature is modeled on.
//
// `launch` is deliberately NOT free-form from callers outside this module:
// the only public entry point (POST /api/tmux/create) never accepts a
// custom command from the browser, because the string below reaches a real
// shell (tmux's shell-command argument is executed via $SHELL -c, so `;` and
// `$SHELL` are meant to be interpreted) — accepting arbitrary text there
// would be shell injection with extra steps. `name` is the only variable
// interpolated into it, and SESSION_NAME_RE has already constrained it to
// `[A-Za-z0-9_.:@-]+`, which contains no shell metacharacters. `cwd` never
// touches the shell string at all — it goes in as tmux's own `-c` argv
// token.
//
...
export async function createSession(name, cwd, opts = {}) {
  ...
  if (typeof cwd !== 'string' || !cwd) return { ok: false, error: 'a working directory is required' };

  let dirStat;
  try { dirStat = await stat(cwd); } catch { return { ok: false, error: `directory does not exist: ${cwd}` }; }
  if (!dirStat.isDirectory()) return { ok: false, error: `not a directory: ${cwd}` };

  if (await hasSession(name)) {
    return { ok: false, status: 409, error: `a tmux session named "${name}" already exists` };
  }

  const launch = opts.launch || `claude --remote-control ${name}`;
  const shellCmd = `${launch}; exec $SHELL`;
  const r = await runTmux(['new-session', '-d', '-s', name, '-c', cwd, shellCmd]);
  if (!r.ok) return { ok: false, error: (r.stderr || 'tmux new-session failed').trim() };
  return { ok: true };
}

// waitAndSendPrompt(name, text): best-effort starter-prompt delivery right
// after createSession(). The freshly spawned process needs a moment to reach
// its own input prompt before typed text means anything to it, and tmux
// gives no external "ready" signal short of polling pane contents — so this
// just retries sendPrompt() with backoff instead. A failure after the last
// attempt is returned, never thrown: whether the prompt lands is independent
// of whether the session itself was created successfully.
const PROMPT_RETRY_DELAYS_MS = [1200, 1500, 2000, 2500]; // ~7.2s worst case

export async function waitAndSendPrompt(name, text) {
  let last = { ok: false, error: 'not attempted' };
  for (const delay of PROMPT_RETRY_DELAYS_MS) {
    await new Promise((r) => setTimeout(r, delay));
    last = await sendPrompt(name, text);
