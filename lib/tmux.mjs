...
async function processTree(panePid, kimiPids) {
  ...
  for (let depth = 0; depth <= PROC_MAX_DEPTH && frontier.length && out.length < PROC_MAX_NODES; depth++) {
    const argvs = await Promise.all(frontier.map(readArgv));
    const next = [];
    for (let i = 0; i < frontier.length && out.length < PROC_MAX_NODES; i++) {
      const argv = argvs[i];
      if (argv === null) continue; // dead or unreadable — skip, never throw
      out.push({ pid: frontier[i], argv, kimiServer: kimiPids.has(frontier[i]) });
...
export async function listSessions() {
  ...
  try {
    const kimiPids = await kimiServerPids();
    await Promise.all(allPanes.map(async (pane) => {
      const processes = await processTree(pane.pid, kimiPids);
  ...
  return { available: true, sessions: Array.from(sessions.values()).map(rollUp) };
}
...
export async function inspectSession(name) {
  ...
  try {
    const kimiPids = await kimiServerPids();
    const processes = await processTree(target.pid, kimiPids);
