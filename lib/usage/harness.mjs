...
export function defineProvider(policy) {
  ...
  let inFlight = null; // coalesce cold/concurrent requests into one compute()
  ...
  async function get({ fresh: wantFresh = false } = {}) {
    ...
    // A cold request is common when the hub first opens, and Promise.all in
    // the route asks for the same provider from several tabs at once. Share
    // that compute promise; otherwise every caller starts its own disk scan or
    // OAuth request before the first one has had a chance to populate cache.
    if (inFlight) {
      const value = await inFlight;
      return marksCached ? { ...value, cached: true } : value;
    }
