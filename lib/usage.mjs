...
async function computeClaudeUsage() {
  ...
  if (!Number.isFinite(expiresAt) || expiresAt <= Date.now()) {
    ...
    return {
      provider: 'claude', status: 'token-expired', plan, tier, windows: [], fetchedAt,
  ...
  return { provider: 'claude', status: 'ok', plan, tier, windows, fetchedAt };
}
...
export async function getClaudeUsage() {
  ...
  claudeCache = { at: now, ttl: value.status === 'ok' ? CLAUDE_OK_TTL_MS : CLAUDE_ERR_TTL_MS, value };
  return value;
}
