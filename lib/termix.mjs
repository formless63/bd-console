...
const TERMIX_RESPONSE_MAX_BYTES = 1024 * 1024;

// Termix normally returns a small JSON list, but it is a user-configured
// endpoint. Read responses through a byte-counted stream so an accidental
// proxy/HTML dump (or a hostile endpoint) cannot make the hub buffer arbitrary
// data before Settings gets its error message.
async function boundedResponseText(res, maxBytes = TERMIX_RESPONSE_MAX_BYTES) {
...
export async function fetchTermixHosts({ baseUrl, token, timeoutMs = 8000 } = {}) {
  ...
  try {
    const body = await boundedResponseText(res);
    if (body == null) throw new Error('response too large or unreadable');
