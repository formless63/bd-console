...
// a shell prompt: it needs the same `~`-expansion and absoluteness checks
// register, the tmux session launcher (bd-console-cox.1) and the file
// upload route (bd-console-cox.2) all take a path from the browser and never
// want to execute anything on the strength of it — only stat()/join() it —
// so this one normalizer backs all three instead of three copies drifting.
// Returns { ok: true, path } or { ok: false, status, error }.
const USER_PATH_MAX = 4096;

export function normalizeUserPath(inputPath) {
  const raw = typeof inputPath === 'string' ? inputPath.trim() : '';
  if (!raw) return { ok: false, status: 400, error: 'path is required' };
  if (raw.includes('\0') || raw.length > USER_PATH_MAX) {
...
// POST /api/register goes through here instead of calling addProject()
// directly. Two rules shape this function:
//
...
export function registerProjectPath(inputPath) {
  const norm = normalizeUserPath(inputPath);
